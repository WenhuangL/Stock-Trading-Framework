...
def calculate_weekly_scores(df: pd.DataFrame, top_n: int, sma200_filter: bool = False, target_hv: float = 0.40) -> dict:
    """
    ...
    sma200_filter: when True only stocks above their 200-day SMA are eligible.
    Requires pad_days >= 250 so SMA has adequate warm-up history.
    target_hv: center of the volatility Gaussian (annualized HV, default 0.40).
    Raise (e.g. 0.55) to bias toward more volatile names for VWAP reversion.
    """
...
def main():
    ...
    if args.short_universe:
        ...
        universe_map = calculate_weekly_scores(df, top_n=args.top_n,
                                              sma200_filter=args.sma200_filter,
                                              target_hv=args.target_hv)
