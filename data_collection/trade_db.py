...
def init_db() -> None:
    ...
    with _connect() as conn:
        ...
        for col, coltype in [
            ...
            ("spy_regime",       "TEXT"),
            ("rsi2_at_entry",    "REAL"),
            ("signal_type",      "TEXT"),   # micro-cap: oversold | gap_sympathy
            ("dollar_vol",       "REAL"),   # micro-cap: 20d avg dollar volume at entry
        ]:
            try:
        ...
        for col, coltype in [
            ...
            ("rsi2_win_rate",       "REAL"),
            ("microcap_final",      "REAL"),
            ("microcap_return_pct", "REAL"),
            ("microcap_trades",     "INTEGER"),
            ("microcap_win_rate",   "REAL"),
            ("microcap_pf_stress",  "REAL"),  # survivorship-stressed profit factor
        ]:
            try:
...
def save_run(
    ...
    microcap_results: Optional[dict] = None,
) -> str:
    """
    ...
    microcap_s     = microcap_results.get("summary", {}) if microcap_results else {}
    ...
    microcap_trades = microcap_results.get("trades", []) if microcap_results else []
    ...
    with _connect() as conn:
        conn.execute("""
            INSERT INTO backtest_runs (
                ...
                microcap_final, microcap_return_pct, microcap_trades, microcap_win_rate,
                microcap_pf_stress
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            ...
            microcap_s.get("final_value"),
            microcap_s.get("total_return_pct"),
            microcap_s.get("num_trades"),
            microcap_s.get("win_rate_pct"),
            microcap_s.get("profit_factor_stress"),
        ))
        ...
        # Micro-cap trades — long-only; store signal_type / decline / dollar_vol.
        micro_rows = []
        for t in microcap_trades:
            entry_p = float(t.get("entry_price") or 0)
            exit_p  = float(t.get("exit_price") or 0)
            pnl_pct = round((exit_p - entry_p) / entry_p * 100, 4) if entry_p > 0 else None
            micro_rows.append((
                run_id, "microcap", "microcap",
                str(t.get("date", "")),
                t.get("symbol"),
        ...
        if micro_rows:
            conn.executemany("""
                INSERT INTO trades (
                    ...
                    bars_held, daily_decline_pct, rsi2_at_entry,
                    signal_type, dollar_vol
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """, micro_rows)
